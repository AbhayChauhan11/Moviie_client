...
export default function Home() {
  ...
  const { createRoom, joinRoom } = useContext(TypeExContext);

  // ✅ Create socket connection only once
  useEffect(() => {
    const newSocket = io("https://moviie-socket-server.onrender.com");
    setSocket(newSocket);
  ...
  // ✅ Get user details and init username
  useEffect(() => {
    const details = async () => {
      try {
  ...
  // ✅ Protect page if no context & no roomId
  useEffect(() => {
    if (!localStorage.getItem("roomId") && !createRoom && !joinRoom) {
      router.push("/");
    }
  }, []);
  ...
  const chandleJoin = async () => {
    if (userName && roomId) {
      try {
        ...
        toast.error("Choose another roomId, room already exists");
      }
    } else {
  ...
  const jhandleJoin = async () => {
    if (userName && roomId) {
      try {
        ...
        toast.error("Room doesn't exist. Press Moveefy to go back.");
      }
    } else {
      toast.error("Please fill in Username and Room Id");
    }
  ...
  return (
    <div className="bg-gradient-to-br from-[#A58EFF] via-[#FF7AC2] to-[#BE76CF] min-h-screen flex flex-col items-center justify-center">
      ...
      {!showChat && (
        <div className="flex flex-col items-center justify-center bg-white/80 dark:bg-black/60 shadow-2xl rounded-3xl p-10 mt-12 max-w-md w-full border border-[#BE76CF]/20 backdrop-blur-md transition-all duration-300">
